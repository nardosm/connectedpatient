...
/**
 The mobile manager is the entry point to the Oracle Mobile Cloud Service SDK.
...
#pragma mark - Analytics App

/**
 The mobile manager's analytics app. This is the analytics app
 configured in the OMC property list file (`OMC.plist`).
 */
@property (readonly, nullable)  OMCCxAAnalyticsApp* analyticsApp;

/**
 Sets and returns the receiver's analytics app,
 initializing it with the specified properties.
 The properties must be in the same form as the `analyticsApp`
 properties in the OMC property list file (`OMC.plist`).
 
 Clears the receiver's analytics app if the specified properties is `nil`.
 
 If the receiver already contains an analytics app, replaces
 it with the newly created analytics app.
 
 @param properties The new analytics app's initial properties.
 */
- (OMCMobileBackend*)setAnalyticsAppWithProperties:(nullable NSDictionary<NSString*, id>*)properties;
...
/**
 ...
 @see OMCMobileManagerOAuthTokenPath
 */
...
/**
 ...
 the Oracle Mobile Cloud Service SDK also logs the headers and bodies of the requests.
 This flag can be either initialized
 from the default property list file (`OMC.plist`) or set directly at
 runtime via this property. If it is not set explicitly, the flag is `NO`.
